...
## 本次修改日志（菜单弹出与背景模糊时序修复）

- 前端：`src/components/DrawerMenu.tsx`
  - 为抽屉增加 `onAnimationComplete` 钩子，菜单滑入完成后才显示右侧背景层。
  - 新增本地状态 `backdropReady` 控制背景层的挂载时机，修复“右侧先变暗/变色”的不自然现象。
  - 背景层改为仅 `backdrop-blur-md`，移除 `bg-black/50` 变暗效果，点击仍可关闭。
