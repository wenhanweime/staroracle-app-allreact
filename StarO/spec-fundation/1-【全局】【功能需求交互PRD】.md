...
2. **回顾交互：** 目前不支持预览标题。点击星星触发**点亮（闪烁）动作**，并遵循以下最终交互语义：
   - 该点亮为“临时高亮（TapHighlight）”：需要跨设备同步，但**最多保留 1 天**；用户多次点击会累积多个高亮区域。
   - 该点亮不会生成“资产星卡”（不写 `stars`，不进入收藏/collection）；若用户从该高亮/灵感卡进入对话并达到觉察阈值，才会生成资产星卡并永久保留。
