...
- 个性化候选交互回退：取消左右滑动“灵感库”交互与候选面板展示，回到“点击 Galaxy 弹出单张灵感星卡”的原交互；候选只作为弹出的卡片内容来源（`StarO/StarO/InspirationCardOverlay.swift`、`StarO/StarO/GalaxyBackgroundView.swift`）。自测：建议 `xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
