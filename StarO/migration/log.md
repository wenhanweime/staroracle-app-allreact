...
- 星卡弹出层级修复：新增顶层窗口 `StarCardOverlayWindowManager`（windowLevel=`alert+1`）承载单张星卡聚焦浮层，使其直接覆盖在 ChatOverlay/InputDrawer 之上；同时移除“显示星卡时下沉 ChatOverlay 到 normal-2”的策略，避免背景露出主页 Galaxy（`StarO/StarO/StarCardOverlayWindowManager.swift`、`StarO/StarO/NativeChatBridge.swift`、`StarO/StarO/RootView.swift`）。自测：`xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
...
- 运维说明（模型配置）：后端 Edge Functions 的模型/温度默认值来自环境变量（`MODEL_ID`、`SUMMARY_MODEL_ID`、`DEFAULT_TEMPERATURE` 等），读取位置为 `staroracle-backend/supabase/functions/chat-send/index.ts`、`staroracle-backend/supabase/functions/star-cast/index.ts`、`staroracle-backend/supabase/functions/_shared/user_long_term_memory.ts`；最快改法是更新 Supabase Edge Functions secrets（Dashboard 或 `supabase secrets set ...`，必要时 redeploy functions）。本仓仅补充追溯记录（`StarO/migration/userpromptrecord.md`）。自测：无（文档/记录变更）。
- 运维说明（Supabase 配置“像乱码”）：`SUPABASE_ANON_KEY`/`SUPABASE_SERVICE_ROLE_KEY`/`SUPABASE_JWT` 等本质是 token（常见为 JWT：`header.payload.signature` 的 base64 片段），看起来不是可读文字属于正常；并且 Dashboard/CLI 的 secrets 可能出于安全只显示遮罩/摘要，无法“读取明文”，只能通过再次 `secrets set` 覆盖更新，或通过 functions logs 验证 `MODEL_ID` 等非敏感项是否生效（对应用户反馈见 `StarO/migration/userpromptrecord.md`）。
- 运维变更（模型切换）：已在远端 Supabase 项目（`pqpudakplohyqpcrlpou`）将 Edge Functions secrets `MODEL_ID` 更新为 `gpt-4.1-mini`；当前 `SUMMARY_MODEL_ID` 为空字符串，后端会自动回退为 `MODEL_ID`，因此 chat-send 的对话与摘要/长期记忆会统一使用新模型。验证：`supabase secrets list --project-ref pqpudakplohyqpcrlpou` 可见 `MODEL_ID` digest 已变化；线上实际生效可通过一次 `chat-send` 调用后的函数结构化日志 `model_id` 字段确认（对应输入记录见 `StarO/migration/userpromptrecord.md`）。
- UI 对齐（对话加载八芒星）：将 AI 空文本加载态的八芒星从“旋转样式/系统紫”改为复用 ChatOverlay 顶部同款八芒星（`StarRayPulseView`，`#8A5FBD` 闪烁动效）；并将 `StarRayPulseView` 改为不再自动 `didMoveToWindow` 启动，统一由调用方显式 `start()`（避免非加载态 cell 也后台动画）（`StarO/StarO/ChatOverlayManager.swift`）。自测：`xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`（建议）。
- 个性化灵感候选（3 条）：新增“基于用户长期记忆”的候选链路——端侧保持预取缓存 3 条并后台补齐；Galaxy 点击仍弹出**单张**灵感星卡，但云端可用时优先使用个性化候选作为卡片正文（不再额外展示候选列表 UI），候选文本风格偏形而上且长度收敛（≤50字）（`StarO/StarO/PersonalizedInspirationService.swift`、`StarO/StarO/PersonalizedInspirationPrefetcher.swift`、`StarO/StarO/PersonalizedInspirationCache.swift`、`StarO/StarO/GalaxyBackgroundView.swift`；核心模型扩展 `PersonalizedInspirationCandidate` 见 `ios-native/Packages/StarOracleCore/Sources/StarOracleCore/Models/Inspiration.swift`）。自测：`xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
- 灵感卡重复率降低：为云端 `star-pluck` 灵感卡注入 `source_id:` tag，并在端侧预取器维护“近期展示 source_id 列表”做去重；同时将云端灵感卡预取从 1 张扩为 2 张缓冲，避免高频点击时因预取未跟上而反复复用同一来源（`StarO/StarO/StarPluckService.swift`、`StarO/StarO/InspirationCardPrefetcher.swift`、`StarO/StarO/InspirationPluckHistoryCache.swift`、`StarO/StarO/GalaxyBackgroundView.swift`）。自测：建议 `xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
- 个性化候选交互回退：仅关闭“灵感卡片界面”左右滑动造成的“切换候选/误触面板”，并将候选稳定为“候选池”（每次 Galaxy 点击只弹出 1 张；其余候选作为下一张/下下一张留在预取池中，后台补齐）；Root 级左右滑动打开菜单/星卡集仍保留（`StarO/StarO/RootView.swift`、`StarO/StarO/InspirationCardOverlay.swift`、`StarO/StarO/PersonalizedInspirationPrefetcher.swift`、`StarO/StarO/GalaxyBackgroundView.swift`）。自测：建议 `xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
- 左侧菜单用户信息对齐：菜单底部头像/昵称不再用 `userEmail` 前缀推导，而是复用 `get-profile` 的 `display_name/avatar_emoji`（与个人主页一致）；并将 profile 快照写入 `AuthService` + `UserDefaults` 缓存，Account 保存/刷新后即时同步，避免“回到左侧菜单仍显示旧头像/旧昵称”的滞后（`StarO/StarO/AuthService.swift`、`StarO/StarO/DrawerMenuView.swift`、`StarO/StarO/AccountView.swift`）。自测：`xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
- 个人主页体验补齐（#46）：新增 `ProfileSnapshotStore` 缓存 `get-profile` 的 user/profile/stats，AccountView 打开先读缓存即时渲染（昵称/头像/注册时间/统计/长期记忆/云端模型等）再异步校验刷新，减少“打开后还要点刷新才显示”的延迟感；并在后端新增 `profiles.preferred_model_id`（迁移已通过管理 API 执行），`chat-send` 上游模型优先使用该字段，AccountView 增加“云端模型”编辑入口（`StarO/StarO/ProfileSnapshotStore.swift`、`StarO/StarO/AccountView.swift`、`StarO/StarO/AuthService.swift`、`StarO/StarO/ProfileService.swift`；后端见 `staroracle-backend/supabase/migrations/20251221220000_profile_preferred_model_id.sql`、`staroracle-backend/supabase/functions/get-profile/index.ts`、`staroracle-backend/supabase/functions/update-profile/index.ts`、`staroracle-backend/supabase/functions/chat-send/index.ts`）。自测：建议 `xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
- 个人头像照片（相册选择）：个人主页支持从相册选择照片头像，端侧裁剪/压缩后上传 Supabase Storage `avatars`（public bucket，用户仅能写入自己目录），并将 `profiles.avatar_url` 写回云端；展示优先级为“照片 > emoji > fallback”，左侧菜单也同步展示；补齐 `NSPhotoLibraryUsageDescription`（`StarO/StarO/AvatarUploadService.swift`、`StarO/StarO/AvatarImageProcessor.swift`、`StarO/StarO/AccountView.swift`、`StarO/StarO/AuthService.swift`、`StarO/StarO/DrawerMenuView.swift`、`StarO/StarO/ProfileService.swift`、`StarO/StarO.xcodeproj/project.pbxproj`；后端见 `staroracle-backend/supabase/migrations/20251221223000_profile_avatar_photo.sql`、`staroracle-backend/supabase/functions/get-profile/index.ts`、`staroracle-backend/supabase/functions/update-profile/index.ts`）。自测：`xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
- 迁移记录补齐：将 `StarO/migration/userpromptrecord.md` 中 #47 的子任务标记为已完成并补齐完成时间，保持“输入→实现→提交→日志”闭环可追溯。
- 会话标题更像“人话”：优化会话标题生成提示词与后处理，避免产出“关于…的对话/聊天/讨论”等模板；并改进 mock/降级路径标题生成，确保即使走兜底也更可读；同时将旧模板标题视为占位并允许自动重算（含本地持久化与云端 chats.title），打开会话/对话完成后会自动生成新标题并回写云端（`StarO/StarO/AppEnvironment.swift`、`StarO/StarO/ConversationStore.swift`、`StarO/StarO/NativeChatBridge.swift`、`ios-native/Packages/StarOracleFeatures/.../ChatStore.swift`、`ios-native/Packages/StarOracleServices/.../MockServices.swift`）。
