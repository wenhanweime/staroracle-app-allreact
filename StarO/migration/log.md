...
- 个性化灵感候选（3 条）：新增“基于用户长期记忆”的候选链路——Galaxy 点击弹出临时灵感卡后，底部同步展示 3 条与用户相关的候选（概念/行动/哲思/提问/小实验/脚本），并在每次点击后后台补齐下一批；端侧本地落盘缓存（10min TTL）避免冷启动空白（`StarO/StarO/PersonalizedInspirationService.swift`、`StarO/StarO/PersonalizedInspirationPrefetcher.swift`、`StarO/StarO/PersonalizedInspirationCache.swift`、`StarO/StarO/PersonalizedInspirationCandidatesPanel.swift`、`StarO/StarO/GalaxyBackgroundView.swift`、`StarO/StarO/InspirationCardOverlay.swift`；核心模型扩展 `PersonalizedInspirationCandidate` 见 `ios-native/Packages/StarOracleCore/Sources/StarOracleCore/Models/Inspiration.swift`，状态注入 `StarStore.personalizedInspirationCandidates` 见 `ios-native/Packages/StarOracleFeatures/Sources/StarOracleFeatures/Stores/StarStore.swift`）。自测：`xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
...
- 个性化候选 UI 对齐：将 3 条候选合并为“灵感库”，在 `InspirationCardOverlay` 内以同款翻转灵感星卡呈现（左右滑动切换卡片）；不再使用独立候选面板（`StarO/StarO/InspirationCardOverlay.swift`）。自测：建议 `xcodebuild -project StarO/StarO.xcodeproj -scheme StarO -sdk iphonesimulator build`。
