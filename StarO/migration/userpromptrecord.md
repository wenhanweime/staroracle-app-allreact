...
- [x] 长期记忆系统未生效：定位原因并修复（端侧触发/接口调用/后端落库/前端注入与展示）（已完成 2025-12-20 23:18）
  - 根因：生产环境未部署 `user-memory-refresh`，端侧虽有 idle 触发逻辑但会 404/无法落库更新。
  - 修复：部署 `user-memory-refresh`；端侧在 restore 后补一次 idle 检查触发；个人中心新增“立即刷新”按钮便于验证与排障。
