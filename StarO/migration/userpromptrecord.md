...
- [ ] 长期记忆系统未生效：定位原因并修复（端侧触发/接口调用/后端落库/前端注入与展示）
  ...
  - [ ] 远端 DB 迁移：`profiles.long_term_memory_*` 字段缺失，导致 `user-memory-refresh` 500（待执行）
  - [ ] 回归验证：`user-memory-refresh` 更新 profiles 字段，个人主页展示更新且 chat-send 注入生效（待执行）
...
- [ ] `user-memory-refresh` 请求失败 500 UM99：定位并修复（远端 migration + 回归验证）（进行中）
