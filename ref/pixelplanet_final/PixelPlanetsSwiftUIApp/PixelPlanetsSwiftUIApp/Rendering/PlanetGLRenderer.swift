...
final class PlanetGLRenderer {
    ...
    init?(planet: PlanetBase) {
        self.planet = planet
        guard EAGLContext.current() != nil else {
            assertionFailure("EAGLContext must be current before creating PlanetGLRenderer.")
            return nil
        }
        setupBuffers()
        rebuildPrograms()
    ...
    func draw(in view: GLKView) {
...
#if DEBUG
        print("📐 Drawable:", view.drawableWidth, "x", view.drawableHeight)
#endif
        ...
        let offsetX = (drawableWidth - square) / 2
        let offsetY = (drawableHeight - square) / 2

        glViewport(offsetX, offsetY, square, square)

        glBindBuffer(GLenum(GL_ARRAY_BUFFER), vertexBuffer)

        let maxLayerPixels = computeMaxLayerPixels()
        ...
        for layer in planet.layers where layer.visible {
            ...
            applyViewport(for: layer, canvasPixels: canvasPixels, maxLayerPixels: maxLayerPixels, offsetX: offsetX, offsetY: offsetY)
